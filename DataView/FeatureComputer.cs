    ...
    class FeatureComputer : IFeatureComputer
    {
        ...
        public List<Point3D> GetSphere(Point3D x, VolumetricData d, double r)
        {
            ...
            for (double i = D.x; i <= C.x; i += d.GetXSpacing())
            {
                for (double j = D.y; j <= A.y; j += d.GetYSpacing())
                {
                    ...
                    {
                        double d2 = (i - x.x) * (i - x.x) + (j - x.y) * (j - x.y) + (k - x.z) * (k - x.z); //tohle nefunguje. odecita se od sebe souradnice realny a prepocitany.
                        //double d2 = (i / d.GetXSpacing() - x.x) * (i / d.GetXSpacing() - x.x) + (j / d.GetYSpacing() - x.y) * (j / d.GetYSpacing() - x.y) + (k / d.GetZSpacing() - x.z) * (k / d.GetZSpacing() - x.z); // tohle taky nefunguje
                        if (d2 <= r * r)
                        {
        ...
        public double[] ComputeFeatureVector(VolumetricData d, Point3D p)
        {
            ...
            for (double r = 0.5; r <= 2.5; r += 0.5) // prumer ?
            {
                ...
                int i = (int)(r / 0.5) - 1;
                featureVector[i] = sum / points.Count; //tady jsem pridal prumer
            }
